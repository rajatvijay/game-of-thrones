...
class Stats:
    ...
    def _get_types_of_battles(self):
        ...
        return [type['battle_type'] for type in type_of_battles if type['battle_type']]
    ...
    def _get_active_region(self):
        return Battle.objects.values_list('region').annotate(region_count=Count('region')).order_by('-region_count')[0][0]

    def _get_defender_stats(self):
        common_defender_size = Battle.objects.values_list('defender_size').annotate(defender_count=Count('defender_size')).order_by('-defender_count')[0][0]
        lowest_defender_list = Battle.objects.filter().values_list('defender_size').annotate(Min('defender_size')).order_by('defender_size')
    ...
    def _get_active_kings(self):
        active_attacker_king_id = Battle.objects.values_list('attacker_king').annotate(king_count=Count('attacker_king')).order_by('-king_count')[0][0]
        active_defender_king_id = Battle.objects.values_list('attacker_king').annotate(king_count=Count('attacker_king')).order_by('-king_count')[0][0]
